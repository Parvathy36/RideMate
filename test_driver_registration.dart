...
Future<void> testDriverRegistrationFlow() async {
  ...
  try {
    ...
    final isLicenseRegistered =
        await FirestoreService.isLicenseAlreadyRegistered(testLicenseId);
    if (isLicenseRegistered) {
      print(
        '⚠️  License is already registered - this is expected for repeated tests',
      );
    } else {
    ...
    if (e.toString().contains('already registered')) {
      ...
      print(
        '   License validation failed - check if sample license data exists',
      );
    }
  }
...
Future<void> testLicenseValidation() async {
  ...
  final validResult = await LicenseValidationService.validateLicense(
    validLicense,
  );
  if (validResult != null) {
  ...
  final invalidResult = await LicenseValidationService.validateLicense(
    invalidLicense,
  );
...
void testLicenseFormatValidation() {
  ...
  final testCases = [
    ...
    {
      'license': 'KL01 202300000001',
      'expected': false,
      'description': 'Extra digits',
    },
  ];
